...
lm.trend=readRDS("/scratch/users/tballard/shum/class.project/shum.hw.trend.USonly.rds")
...
#Ventura et al. (2004) with q=.01
  ...
  ##performs the adjustment Ventura et al. suggest to that method if using spatial data.
  ##Output is a vector of the indices of the significant tests
  pval.jan5.fdr=fdr(pval.jan5.ii, qlevel=0.01, method="original", adjustment.method="mean") 
  length(pval.jan5.fdr) #length=
  #Now set the indices not present in that output to NA
  pval.jan5.ii.ind=rep(0,length(pval.jan5.ii))
  pval.jan5.ii.ind[pval.jan5.fdr]=1 #Vector of 0's and 1's
  pval.jan5.ii.adj=pval.jan5.ii.ind*pval.jan5.ii #vector length ... of original p-vals and 0's
  ...
  pval.jul5.fdr=fdr(pval.jul5.ii, qlevel=0.01, method="original", adjustment.method="mean") 
  length(pval.jul5.fdr) #length=
  #Now set the indices not present in that output to NA
  pval.jul5.ii.ind=rep(0,length(pval.jul5.ii))
  pval.jul5.ii.ind[pval.jul5.fdr]=1 #Vector of 0's and 1's
  pval.jul5.ii.adj=pval.jul5.ii.ind*pval.jul5.ii #vector length ... of original p-vals and 0's
...
##### Save adjusted p-values to combined array #####
  ...
  saveRDS(pvals,"/scratch/users/tballard/shum/class.project/pvals.USonly.rds")
...
##### Some summary stats on the correction methods results #####
  sum(pval.jan<.05, na.rm=T) #221 significant pval's originally
  sum(pval.jan2<.05, na.rm=T)/sum(pval.jan<.05, na.rm=T) #62% remain significant
  sum(pval.jan3<.05, na.rm=T)/sum(pval.jan<.05, na.rm=T) #33%
  sum(pval.jan4<.05, na.rm=T)/sum(pval.jan<.05, na.rm=T) #100%
  sum(pval.jan5<.05, na.rm=T)/sum(pval.jan<.05, na.rm=T) #53%
  sum(pval.jan6<.05, na.rm=T)/sum(pval.jan<.05, na.rm=T) #19%
  
  
  sum(pval.jul<.05, na.rm=T) #394 significant pval's originally
  sum(pval.jul2<.05, na.rm=T)/sum(pval.jul<.05, na.rm=T) #94% remain
  sum(pval.jul3<.05, na.rm=T)/sum(pval.jul<.05, na.rm=T) #78%
  sum(pval.jul4<.05, na.rm=T)/sum(pval.jul<.05, na.rm=T) #100%
  sum(pval.jul5<.05, na.rm=T)/sum(pval.jul<.05, na.rm=T) #94%
  sum(pval.jul6<.05, na.rm=T)/sum(pval.jul<.05, na.rm=T) #58%