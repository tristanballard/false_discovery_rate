...
##Only difference between this code and the original is that it reads in the trend results from the Rank regression
...
##### Save adjusted p-values to combined array #####
  ...
  saveRDS(pvals,"/scratch/users/tballard/shum/class.project/pvals.rank.rds")
...
##### Some summary stats on the correction methods results #####
  sum(pval.jan<.05, na.rm=T) #2487 significant pval's originally
  sum(pval.jan2<.05, na.rm=T)/sum(pval.jan<.05, na.rm=T) #78% remain significant
  sum(pval.jan3<.05, na.rm=T)/sum(pval.jan<.05, na.rm=T) #40%
  sum(pval.jan4<.05, na.rm=T)/sum(pval.jan<.05, na.rm=T) #100%
  sum(pval.jan5<.05, na.rm=T)/sum(pval.jan<.05, na.rm=T) #64%
  sum(pval.jan6<.05, na.rm=T)/sum(pval.jan<.05, na.rm=T) #17%
  
  
  sum(pval.jul<.05, na.rm=T) #2686 significant pval's originally
  sum(pval.jul2<.05, na.rm=T)/sum(pval.jul<.05, na.rm=T) #90% remain
  sum(pval.jul3<.05, na.rm=T)/sum(pval.jul<.05, na.rm=T) #65%
  sum(pval.jul4<.05, na.rm=T)/sum(pval.jul<.05, na.rm=T) #99%
  sum(pval.jul5<.05, na.rm=T)/sum(pval.jul<.05, na.rm=T) #75%
